...
Tools: `list_meals`, `get_meal`, `list_daily_metrics`, `glucose_summary`, and `doctor_report`. All of them are read-only; logging and editing stay in the app. To disconnect, remove the server from the client (`claude mcp remove diabuddy -s user`).
...
3. Deploy the function and apply the migrations:
...
- `mcp` runs every tool under the caller's JWT, through PostgREST or a Postgres connection with the caller's claims set, so an MCP client sees exactly what that user sees in the app. The function holds no admin credentials.
